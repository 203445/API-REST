...
const urlGet = 'http://localhost:28080/listMascota'
...
const urlDel = 'http://localhost:28080/mascota/delete'
    ...
    const deleteMascota = (dat,mascotaId) => {
        let isDelete = window.confirm(`¿Estás seguro de eliminar la mascota con ID ${mascotaId} ?`);
        if (isDelete) {
            const peticion = {
                method: 'DELETE',
                body: JSON.stringify(dat),
